...
def lab_index(session_id):
    ...
    return render_template("index.html", username=data['username'], seconds=(data['max_date'] - datetime.datetime.now()).seconds)
...
@app.route("/microscope/weblab/sessions/", methods=['POST'])
def start_experiment():
    ...
    link = url_for('lab_index', session_id=session_id, _external = True)
...
# user.
# 
@app.route('/microscope/weblab/sessions/<session_id>/status')
...
# is over.
# 
@app.route('/microscope/weblab/sessions/<session_id>', methods=['POST'])
